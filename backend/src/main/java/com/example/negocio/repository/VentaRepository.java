...
public interface VentaRepository extends JpaRepository<Venta, Long>, JpaSpecificationExecutor<Venta> {
        ...
        @Query(value = "SELECT metodoDePago AS etiqueta, COUNT(*) AS valor FROM venta\n" +
                        "WHERE fechaHora >= :fechaInicio AND fechaHora < :fechaFin\n" +
                        "GROUP BY metodoDePago", nativeQuery = true)
        ...
        @Query(value = "SELECT IFNULL(SUM(total), 0) FROM venta " +
                        ...
                        "  AND fechaHora < DATE_FORMAT(CURDATE(), '%Y-%m-01') + INTERVAL 1 MONTH", nativeQuery = true)
        ...
        @Query(value = "SELECT IFNULL(SUM(total), 0) " +
                        "FROM venta " +
                        "WHERE fechaHora >= :fechaInicio AND fechaHora < :fechaFin ", nativeQuery = true)
        BigDecimal findRecaudadoVentasDiarias(
                        @Param("fechaInicio") LocalDateTime fechaInicio,
                        @Param("fechaFin") LocalDateTime fechaFin);

        @Query(value = "SELECT COALESCE(SUM(montoAdicional), 0) FROM venta WHERE fechaHora BETWEEN :fechaInicio AND :fechaFin", nativeQuery = true)
        BigDecimal sumMontoAdicionalPorFechas(@Param("fechaInicio") LocalDateTime fechaInicio,
                        @Param("fechaFin") LocalDateTime fechaFin);

        @Query("SELECT new com.example.negocio.dto.reporte.ReporteMetodoDePagoDTO(v.metodoDePago, SUM(v.total)) " +
                        "FROM Venta v " +
                        "WHERE v.fechaHora BETWEEN :fechaInicio AND :fechaFin " +
                        "GROUP BY v.metodoDePago")
        List<ReporteMetodoDePagoDTO> obtenerTotalesPorMetodoPago(@Param("fechaInicio") LocalDateTime fechaInicio,
                        @Param("fechaFin") LocalDateTime fechaFin);
}
